...
export class TacticPageValidator {
  static validate(page: unknown): ValidationResult {
    ...
    if (tacticPage.backgroundImage) {
      if (typeof tacticPage.backgroundImage !== 'object' || !tacticPage.backgroundImage.src) {
        errors.push(new ValidationError('backgroundImage must have a valid src property', 'backgroundImage'));
      }
    }
  ...
  private static validateObject(obj: unknown, path: string): ValidationError[] {
    ...
    if (!fabricObj.type || typeof fabricObj.type !== 'string') {
      errors.push(new ValidationError('Object must have a valid type', `${path}.type`));
    }

    // Validate coordinates if present
    if (fabricObj.left !== undefined && (typeof fabricObj.left !== 'number' || !isFinite(fabricObj.left))) {
      errors.push(new ValidationError('left coordinate must be a finite number', `${path}.left`));
    }

    if (fabricObj.top !== undefined && (typeof fabricObj.top !== 'number' || !isFinite(fabricObj.top))) {
      errors.push(new ValidationError('top coordinate must be a finite number', `${path}.top`));
    }

    // Validate specific object types
    switch (fabricObj.type) {
      case 'text':
        if (fabricObj.text !== undefined && typeof fabricObj.text !== 'string') {
          errors.push(new ValidationError('text property must be a string', `${path}.text`));
        }
        break;
      case 'path':
        if (fabricObj.path !== undefined && 
          ...
          errors.push(new ValidationError('path property must be a string or array', `${path}.path`));
        }
        break;
      case 'group':
        if (fabricObj.objects) {
          if (!Array.isArray(fabricObj.objects)) {
            errors.push(new ValidationError('group objects must be an array', `${path}.objects`));
          } else {
            fabricObj.objects.forEach((groupObj, index) => {
              const groupObjErrors = this.validateObject(groupObj, `${path}.objects[${index}]`);
  ...
  static sanitize(page: TacticPage): TacticPage {
    ...
    if (sanitized.objects) {
      sanitized.objects = sanitized.objects
        ...
        .filter(obj => obj !== null && obj.uuid) as any;
    }

    return sanitized;
  }

  private static sanitizeObject(obj: FabricObjectData): FabricObjectData | null {
    if (!obj || !obj.type) {
      return null;
    }
