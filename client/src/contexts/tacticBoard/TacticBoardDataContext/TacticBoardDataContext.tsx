        ...
        try {
          ...
          try {
            if (objData.type === "circle") {
              ...
              const addObj = new fabric.Path(objData.path?.toString(), objData as object);
              canvasFabric.add(addObj);
            } else if (objData.type === "text") {
              if (objData.text) {
                const addObj = new fabric.Text(objData.text, objData as object);
                canvasFabric.add(addObj);
              }
            }
          } catch (fallbackError) {
            console.error('Fallback object creation also failed:', fallbackError);
          }
        }
      });
