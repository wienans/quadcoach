...
async function loadAccessContext(planId: string, userId: string): Promise<AccessContext> {
  const plan = await PracticePlan.findById(planId);
  if (!plan) return { plan: null, isOwner: false, hasEdit: false, hasAnyAccess: false };
  const isOwner = plan.user.toString() === userId;
  if (isOwner) return { plan, isOwner: true, hasEdit: true, hasAnyAccess: true };
  const access = await PracticePlanAccess.findOne({ practicePlan: plan._id, user: userId });
  if (!access) return { plan, isOwner: false, hasEdit: false, hasAnyAccess: false };
  return { plan, isOwner: false, hasEdit: access.access === 'edit', hasAnyAccess: true };
}
...
export const getPracticePlan = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (!ctx.plan || !ctx.hasAnyAccess) return res.status(404).json({ message: 'Not found' });
...
export const patchPracticePlan = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const id = req.params.id;
    const ctx = await loadAccessContext(id, userId);
    if (!ctx.plan || !(ctx.isOwner || ctx.hasEdit)) return res.status(404).json({ message: 'Not found' });
    ...
    if (req.body.sections !== undefined) {
      ...
      if (durationErrors.length) return sendValidation(res, 'Duration validation failed', durationErrors);
      updates.sections = req.body.sections;
    }
    updates.updatedAt = new Date();
    const updated = await PracticePlan.findByIdAndUpdate(ctx.plan._id, updates, { new: true });
...
export const deletePracticePlan = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const id = req.params.id;
    const ctx = await loadAccessContext(id, userId);
    if (!ctx.plan || !ctx.isOwner) return res.status(404).json({ message: 'Not found' });
...
export const addAccess = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const accessList = await PracticePlanAccess.find({ practicePlan: ctx.plan._id });
...
export const removeAccess = async (req: AuthedRequest, res: Response) => {
  try {
    ...
    await PracticePlanAccess.deleteOne({ _id: accessId, practicePlan: ctx.plan._id });
    const accessList = await PracticePlanAccess.find({ practicePlan: ctx.plan._id });
    return res.json({ access: accessList });
  } catch (e: any) {
